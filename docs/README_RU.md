...
## 🚀 Четыре шага для взлета

### 1. Установить CoBridge

Откройте маркетплейс Open VSX, найдите [**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge) и нажмите кнопку установки. Это так просто.

После установки взгляните на строку состояния в правом нижнем углу — вы увидите иконку, предлагающую выбрать Agent.
![Установленная иконка](../public/assets/installed.png)

Нажатие на эту иконку открывает меню, где вы можете:
...
- **Выбрать Agent** (Переключить Agent)

### 2. Выбрать Agent

Откройте меню и выберите используемый Agent.

![Меню управления](../public/assets/management.png)

В настоящее время поддерживаются **6** типов программных Agentов:

![Выбрать Agent](../public/assets/select_agent.png)

После выбора в строке состояния отобразится соответствующая иконка Agentа:

![Agent выбран](../public/assets/agent_selected.png)

### 3. Запустить сервис

Вернитесь в меню и нажмите **Запустить сервис**. Сервис будет работать непрерывно до тех пор, пока вы не остановите его вручную.

![Статус работы](../public/assets/running.png)

### 4. Начать «Телепортацию памяти»
