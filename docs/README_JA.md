...
Open VSXマーケットプレイスを開き、[**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge) を検索してインストールをクリックします。とても簡単です。

インストール後、右下のステータスバーにアイコンが表示され、Agentを選択するよう促されます。
![インストールアイコン](../public/assets/installed.png)

このアイコンをクリックすると、以下のメニューが開きます：
- 手動でサービスの **オン/オフ**
...
メニューを開き、使用するAgentを選択します。

![管理メニュー](../public/assets/management.png)

現在 **6** 種類のプログラミングAgentをサポートしています：

![Agentを選択](../public/assets/select_agent.png)

Agentを選択すると、ステータスバーに対応するAgentアイコンが表示されます：

![Agent選択済み](../public/assets/agent_selected.png)

### 3. サービスを起動

メニューに戻り、サービスの起動をクリックします。サービスは手動で停止するまで継続的に実行されます。

![実行中ステータス](../public/assets/running.png)

### 4. 「記憶の転送」を開始

ブラウザ側の **Voyager** で「コンテキスト同期」機能が有効になっていることを確認してください。**IDEに同期** をクリックすると、会話の内容が自動的に以下のファイルに保存されます：
...
|------|------|
| **VS Code** | `1.50.0` 以上 |
| **ブラウザ拡張機能** | 会話を取得するために、対応する [Voyager](https://github.com/Nagi-ovo/gemini-voyager) が必要です |
...
**大規模言語モデル（LLM）はもう記憶喪失にはなりません。Webで解決策を徹底的に議論し、Agentで直接実装しましょう。**
...
このプロジェクトがあなたのAIコラボレーションの悩みを解決したなら、ぜひ飲み物を一杯おごってください！🥤

あなたのサポートは、プロジェクトの今後のアップデートの維持に直接役立てられます❤️。

<div align="center">

  <p><b>WeChat / Alipay / Afdian でスポンサーする:</b></p>
