...
打开 Open VSX 插件市场，搜索 [**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge)，点击安装。就这么简单。

安装完毕后，瞄一眼右下角状态栏，出现图标提示你选择 Agent
![图标](../public/assets/installed.png)
...
![菜单](../public/assets/management.png)

目前已支持 **6** 种编程 Agent：

![选择 Agent](../public/assets/select_agent.png)

选好 Agent 后，状态栏会显示对应的 Agent 图标：

![选好 Agent](../public/assets/agent_selected.png)



### 3. 开启服务
回到菜单，点击开启服务，服务会一直运行，直到你手动关闭

![running](../public/assets/running.png)

### 4. 开始"记忆搬运"

确保浏览器端的 **Voyager** 已开启"上下文同步"功能。点击 **同步到IDE**，对话内容就会自动落地到：

```
.cobridge/AI_CONTEXT.md
```

从此，你的 Agent 再也不会一脸茫然地问你“你之前说了什么？”
...
**由于 vscode 会以工作区设置覆盖用户设置，所以请在工作区设置中修改端口号**

![端口设置](../public/assets/port.png)
...
| **浏览器插件** | 需要配套的 [Voyager](https://github.com/Nagi-ovo/gemini-voyager) 来抓取对话 |
...
- **格式友好**：全 Markdown 输出， Agent 读起来就像读说明书一样丝滑。
- **自动配置**：它还会自动生成适配各类 Agent 的规则文件，让各路 Agent 无缝读取上下文，不再臃肿。
...
**大模型从此不再失忆，在网页端聊透方案，用 Agent 直接落地。**
...
如果这个项目解决了你的 AI 协作痛点，欢迎请我喝杯快乐水！🥤

你的支持将直接用于维持项目的后续迭代❤️。

<div align="center">
  
  <p><b>通过 微信 / 支付宝 / 爱发电 打赏:</b></p>
