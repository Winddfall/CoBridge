...
Open VSX 마켓플레이스를 열고 [**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge)를 검색하여 설치를 클릭하세요. 정말 간단합니다.

설치 후 오른쪽 하단 상태 표시줄을 확인하세요 — Agent를 선택하라는 아이콘이 표시됩니다.
![설치 아이콘](../public/assets/installed.png)

이 아이콘을 클릭하면 다음 메뉴가 열립니다:
- 서비스 수동 **시작/중지**
...
메뉴를 열고 사용 중인 Agent를 선택하세요.

![관리 메뉴](../public/assets/management.png)

현재 **6**종의 프로그래밍 Agent를 지원합니다:

![Agent 선택](../public/assets/select_agent.png)

Agent를 선택하면 상태 표시줄에 해당 Agent 아이콘이 표시됩니다:

![Agent 선택됨](../public/assets/agent_selected.png)

### 3. 서비스 시작

메뉴로 돌아가 **서비스 시작**을 클릭하세요. 서비스는 수동으로 중지할 때까지 계속 실행됩니다.

![실행 중 상태](../public/assets/running.png)

### 4. "기억 순간이동" 시작

브라우저의 **Voyager**에서 "컨텍스트 동기화"가 활성화되어 있는지 확인하세요. **Sync to IDE**를 클릭하면 대화 내용이 자동으로 다음 위치에 저장됩니다:
...
|------|------|
| **VS Code** | `1.50.0` 이상 |
| **브라우저 확장** | 대화를 캡처하려면 컴패니언 [Voyager](https://github.com/Nagi-ovo/gemini-voyager)가 필요합니다 |
...
이 프로젝트가 귀하의 AI 협업 문제를 해결했다면, 기꺼이 음료 한 잔을 대접해 주세요! 🥤

귀하의 지원은 프로젝트의 후속 개발을 유지하는 데 직접 사용됩니다❤️.

<div align="center">
  
  <p><b>WeChat / Alipay / Afdian 을 통해 후원:</b></p>
