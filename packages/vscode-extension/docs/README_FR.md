<div align="center">

# <img src="../public/icon.svg" width="15%" alt="" align=center /> Synchronisation Mémoire IA Inter-Dimensions ✨

**Pensez dans le navigateur. Concrétisez dans l'éditeur.**

Votre Agent disposera désormais d'une mémoire à long terme transcendant les onglets.

![Gemini](https://img.shields.io/badge/Gemini-✓-4285F4?style=flat-square&logo=googlegemini&logoColor=white)
![ChatGPT](https://img.shields.io/badge/ChatGPT-✓-000?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![Claude](https://img.shields.io/badge/Claude-✓-D97757?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![deepseek](https://img.shields.io/badge/DeepSeek-✓-4D6BFE?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![豆包](https://img.shields.io/badge/Doubao-✓-A569FF?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)

![github copilot](https://img.shields.io/badge/GitHub%20Copilot-✓-6f42c1?style=flat-square&logo=githubcopilot)
![cursor](https://img.shields.io/badge/Cursor-✓-000000?style=flat-square&logo=cursor)
![antigravity](https://img.shields.io/badge/Antigravity-✓-4285F4?style=flat-square&logo=data:image/svg+xml;base64,)
![trae](https://img.shields.io/badge/Trae-✓-32f08C?style=flat-square&logo=data:image/svg+xml;base64,)
![claude code](https://img.shields.io/badge/Claude%20Code-✓-D97757?style=flat-square&logo=data:image/svg+xml;base64,)
![codex](https://img.shields.io/badge/Codex-✓-5865F2?style=flat-square&logo=data:image/svg+xml;base64,)

[![Version](https://img.shields.io/open-vsx/v/windfall/co-bridge?label=release&style=flat-square)](https://open-vsx.org/extension/windfall/co-bridge)
[![Open VSX Installs](https://img.shields.io/open-vsx/dt/windfall/co-bridge?style=flat-square&label=Open%20VSX%20users)](https://open-vsx.org/extension/windfall/co-bridge)
[![VS Code](https://img.shields.io/badge/users-729%20-blue?style=flat-square&logo=data:image/svg+xml;base64,&logoColor=white)](https://marketplace.visualstudio.com/items?itemName=windfall.co-bridge)
[![License](https://img.shields.io/github/license/Winddfall/CoBridge?style=flat-square)](https://github.com/Winddfall/CoBridge/blob/main/LICENSE)
[![GitHub stars](https://img.shields.io/github/stars/Winddfall/CoBridge?style=flat-square&logo=github)](https://github.com/Winddfall/CoBridge/stargazers)

[English](../README.md) | [简体中文](README_CN.md) | [繁體中文](README_ZH_TW.md) | [日本語](README_JA.md) | [Français](README_FR.md) | [Español](README_ES.md) | [Português](README_PT.md) | [한국어](README_KO.md) | [Русский](README_RU.md) | [العربية](README_AR.md)

</div>

> [!NOTE]
> CoBridge prend également en charge le plugin navigateur compagnon [Voyager](https://github.com/Nagi-ovo/gemini-voyager), spécialement conçu pour **Gemini**. Si vous utilisez uniquement Gemini, Voyager est une alternative légère. Pour une prise en charge plus large des plateformes (ChatGPT, Claude, DeepSeek, Doubao), utilisez le plugin navigateur intégré de CoBridge.

---

## ✨ Fonctionnalités

### 📝 Extraction intelligente du contexte
Extrait le contexte des conversations IA depuis les pages web, y compris le texte, les tableaux et les images

### 🎯 Complétion automatique des prompts système
Remplit automatiquement les prompts système pour les Agents de l'espace de travail, démarrant rapidement les projets

### 🌿 Large support d'écosystème
Prend en charge ChatGPT, Claude, DeepSeek, Doubao et d'autres plateformes d'IA

Prend en charge GitHub Copilot, Cursor, Claude Code et d'autres Agents d'IDE

## 📥 Installation
### Installation depuis la boutique
#### Étape 1 : Installer l'extension navigateur
Installez depuis le [Chrome Web Store]() (bientôt disponible)
#### Étape 2 : Installer l'extension VS Code
Ouvrez l'Open VSX Marketplace, recherchez [**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge), puis cliquez sur Installer.

### Installation manuelle
#### Installer l'extension navigateur
1. Téléchargez ou clonez ce dépôt
2. Exécutez `pnpm install && pnpm --filter chrome-extension build` à la racine du projet
3. Ouvrez `chrome://extensions/`, activez le **mode développeur**
4. Cliquez sur **Charger l'extension non empaquetée** et sélectionnez `packages/chrome-extension/dist`

---

## 🏗️ Architecture

```
┌─────────────────────┐         ┌─────────────────────┐
│  Extension           │  HTTP   │   Extension          │
│   Navigateur         │ ──────► │    VS Code           │
│   (Chrome / Edge)    │ :3030   │  (Récepteur Agent)   │
│                      │         │                      │
│  Capture les         │         │  Sauvegarde dans     │
│  conversations IA    │         │  .cobridge/AI_CONTEXT│
└─────────────────────┘         └──────────────────────┘
```

- **Extension navigateur** : injecte du code dans les pages de chat IA, extrait le contenu des conversations (texte, tableaux, images), et envoie les données au serveur local
- **Extension VS Code** : exécute un serveur HTTP local, reçoit les données et les écrit dans des fichiers de contexte au format Markdown, lisibles par l'Agent de l'IDE

---

## 🚀 Démarrage rapide
### Étape 1 : Choisir votre Agent
![Icône installée](../public/assets/installed.png)

Cliquez sur l'icône de la barre d'état pour ouvrir le menu de gestion :

![Menu de gestion](../packages/vscode-extension/public/assets/management.png)

Sélectionnez l'Agent que vous utilisez :

![Sélection de l'Agent](../packages/vscode-extension/public/assets/select_agent.png)

Après la sélection, l'icône de l'Agent correspondant apparaît dans la barre d'état :

![Agent sélectionné](../packages/vscode-extension/public/assets/agent_selected.png)

### Étape 2 : Démarrer le service

Dans le menu, cliquez sur **Démarrer le service**. Le serveur local s'exécute sur le port `3030` jusqu'à l'arrêt manuel.

![État en cours d'exécution](../packages/vscode-extension/public/assets/running.png)

### Étape 3 : Synchroniser les conversations
![同步对话](../public/assets/browser_extension_en.png)

1. Ouvrez n'importe quelle page de chat IA prise en charge dans le navigateur
2. Cliquez sur l'icône de l'extension CoBridge
3. Cliquez sur **Sync Context to Agent**
4. Le contenu de la conversation est automatiquement écrit dans `.cobridge/AI_CONTEXT.md` de votre espace de travail

Désormais, votre Agent ne vous demandera plus jamais d'un air perdu : « Qu'est-ce que vous avez dit précédemment ? »

---

## ⚙️ Configuration

### Modifier le port

Si le port `3030` est déjà utilisé :

1. Ouvrez les paramètres VS Code (`Ctrl + ,` / `Cmd + ,`)
2. Recherchez `AIContextSync.port`
3. Modifiez le numéro de port (par ex. `3031`)
4. Redémarrez le service depuis le menu de la barre d'état

![Paramètres du port](../packages/vscode-extension/public/assets/port.png)

> **Remarque :** Les paramètres de l'espace de travail VS Code remplacent les paramètres utilisateur. Si vous souhaitez modifier le port, faites-le dans les paramètres de l'espace de travail.

La fenêtre popup de l'extension navigateur permet également de configurer le port. Assurez simplement que les deux côtés correspondent.

---

## 🌐 Plateformes IA prises en charge
![Gemini](https://img.shields.io/badge/Gemini-4285F4?style=for-the-badge&?style=flat-square&logo=googlegemini&logoColor=white)
![ChatGPT](https://img.shields.io/badge/ChatGPT-000000?style=for-the-badge&?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![Claude](https://img.shields.io/badge/Claude-D97757?style=for-the-badge&?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![deepseek](https://img.shields.io/badge/DeepSeek-4D6BFE?style=for-the-badge&logo=data:image/svg+xml;charset=utf-8;base64,)
![豆包](https://img.shields.io/badge/Doubao-A569FF?style=for-the-badge&?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
|  | Statut | Description |
|------------|--------|-------------|
| **Tableaux** | ✅ | Conversion automatique en Markdown |
| **Images** | ✅ | Sauvegarde automatique en local |
| **Pièces jointes** | ❌ | Non pris en charge pour le moment |
| **Autres plateformes** | ❌ | Les plateformes avec anti-scraping strict ne sont pas prises en charge (PR bienvenues !) |

---

## 🤖 Agents pris en charge

![VS Code](https://img.shields.io/badge/Visual%20Studio%20Code-007ACC?style=for-the-badge&logo=data:image/svg+xml;base64,)
![GitHub Copilot](https://img.shields.io/badge/GitHub%20Copilot-6f42c1?style=for-the-badge&logo=githubcopilot&logoColor=white)
![Cursor](https://img.shields.io/badge/Cursor-000000?style=for-the-badge&logo=cursor&logoColor=white)
![Trae](https://img.shields.io/badge/Trae-32f08C?style=for-the-badge&logo=data:image/svg+xml;base64,)
![Antigravity](https://img.shields.io/badge/Antigravity-4285F4?style=for-the-badge&logo=data:image/svg+xml;base64,)
![Claude Code](https://img.shields.io/badge/Claude%20Code-D97757?style=for-the-badge&logo=data:image/svg+xml;base64,)
![Codex](https://img.shields.io/badge/Codex-5865F2?style=for-the-badge&logo=data:image/svg+xml;base64,)

---

## 🎯 Principes de conception

- **Zéro pollution** : CoBridge ajoute automatiquement les fichiers synchronisés au `.gitignore`, garantissant qu'ils ne polluent jamais votre dépôt Git.
- **Format convivial** : sortie Markdown complète — tableaux, images, texte sont tous conservés, lisibles naturellement par l'Agent de l'IDE.
- **Configuration automatique** : génération automatique de fichiers de règles adaptés à chaque Agent, permettant une lecture fluide du contexte.
- **Local d'abord** : toutes les données restent sur votre machine, aucun serveur externe, aucune télémétrie.

---

## 📁 Structure du projet

C'est un Monorepo [pnpm workspace](https://pnpm.io/workspaces) contenant deux sous-packages :

```
CoBridge/
├── packages/
│   ├── chrome-extension/     # Extension navigateur (Chrome / Edge)
│   └── vscode-extension/     # Extension VS Code (récepteur Agent)
├── pnpm-workspace.yaml
└── package.json
```

---

## 🛠️ Développement

### Prérequis

- [Node.js](https://nodejs.org/) >= 20
- [pnpm](https://pnpm.io/) >= 8

### Installation

```bash
git clone https://github.com/Winddfall/CoBridge.git
cd CoBridge
pnpm install
```

### Build de l'extension navigateur

```bash
pnpm --filter chrome-extension build
```

### Build de l'extension VS Code

```bash
pnpm --filter vscode-extension compile
```

---

## 🌟 Résumé en une phrase

**Les LLM n'oublient plus jamais : réfléchissez pleinement sur le web, laissez l'Agent concrétiser directement.**

Si ce projet vous a été utile, n'hésitez pas à nous mettre une étoile sur [GitHub](https://github.com/Winddfall/CoBridge) ⭐.

## 💡 Retour d'information

Si vous avez de nouvelles demandes, n'hésitez pas à créer une Issue sur [GitHub](https://github.com/Winddfall/CoBridge/issues).

## 🤝 Contribuer

Si vous avez de bonnes suggestions ou avez trouvé un Bug, n'hésitez pas à soumettre une Pull Request !

## 🥠 Soutenir le projet
<div align="center">
  <a href="https://github.com/Winddfall/CoBridge">
    <img src="https://raw.githubusercontent.com/winddfall/CoBridge/main/public/assets/sponsors.svg" width="1000px" />
  </a>
</div>
Si ce projet a résolu vos problèmes de collaboration IA, offrez-moi un café ! ☕

Votre soutien sera directement investi dans les futures itérations du projet ❤️.

<div align="center">

  <p><b>Soutenir via WeChat / Alipay / Afdian :</b></p>
  <table align="center" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center">
        <img src="../packages/vscode-extension/public/assets/wechat-sponsor.png" alt="WeChat Pay" height="160"><br>
        <sub><b>WeChat Pay</b></sub>
      </td>
      <td align="center">
        <img src="../packages/vscode-extension/public/assets/alipay-sponsor.jpg" alt="Alipay" height="160"><br>
        <sub><b>Alipay</b></sub>
      </td>
      <td align="center">
        <a href="https://afdian.com/a/Wind_fall" target="_blank"><b>Afdian</b></a><br>
        <sub><b>Afdian</b></sub>
      </td>
    </tr>
  </table>
</div>

## 📄 Licence

Ce projet est sous licence MIT.

## Tendance des étoiles

<a href="https://www.star-history.com/?repos=Winddfall%2FCoBridge&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/image?repos=Winddfall/CoBridge&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/image?repos=Winddfall/CoBridge&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/image?repos=Winddfall/CoBridge&type=date&legend=top-left" />
 </picture>
</a>
