...
# <img src="../public/icon.svg" width="15%" alt="" align=center /> 跨越次元的 AI 记忆同步 ✨

**浏览器里思考，编辑器里落地。**

你的 Agent，从此拥有跨越标签页的长期记忆。
...
> [!NOTE]
> CoBridge 同样支持配套的浏览器插件 [Voyager](https://github.com/Nagi-ovo/gemini-voyager)，它专门为 **Gemini** 设计。如果你只使用 Gemini，Voyager 是一个轻量级的替代方案。如需更广泛的平台支持（ChatGPT、Claude、DeepSeek、豆包），请使用 CoBridge 浏览器插件。

---

## ✨ 功能特性
### 📝 智能提取上下文
提取网页版对话上下文，包括文本、表格、图片

### 🎯 系统提示词补全
自动填写工作区 Agent 的系统提示词，快速落地项目

### 🌿 超多生态支持
支持 ChatGPT、Claude、DeepSeek、豆包等多种 AI 平台

支持 Github Copilot、Cursor、Claude Code 等IDE Agent

## 📥 安装方式
### 商店安装
#### 第一步：安装浏览器插件
从 [Chrome 应用商店]() 安装（即将上线）
#### 第二步：安装 VS Code 插件
打开 Open VSX 应用商店，搜索 [**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge)，点击安装。

### 手动安装
#### 安装浏览器插件
1. 下载或克隆本仓库
2. 在项目根目录运行 `pnpm install && pnpm --filter chrome-extension build`
...
- **浏览器插件**：注入 AI 聊天页面，提取对话内容（文本、表格、图片），发送到本地服务器
- **VS Code 插件**：运行本地 HTTP 服务器，接收数据并写入 Markdown 格式的上下文文件，供 IDE Agent 读取

---

## 🚀 快速开始
### 第一步：选择你的 Agent
![状态栏](../public/assets/installed.png)
...
选择后，状态栏会显示对应的 Agent 图标：

![Agent 已选中](../public/assets/agent_selected.png)

### 第二步：启动服务

在菜单中点击**启动服务**。本地服务器将在 `3030` 端口运行，直到你手动停止。

![运行状态](../public/assets/running.png)

### 第三步：同步对话

![同步对话](../public/assets/browser_extension_cn.png)

1. 在浏览器中打开任意支持的 AI 聊天页面
2. 点击 CoBridge 浏览器插件图标
3. 点击 **同步上下文至 Agent**
4. 对话内容将自动写入工作区的 `.cobridge/AI_CONTEXT.md`
...
> **注意：** VS Code 工作区设置会覆盖用户设置。如需修改端口，请在工作区设置中修改。

浏览器插件的弹窗也可以设置端口，保持两端一致即可。
...
| 格式 | 状态 | 说明 |
|------|------|------|
| **表格支持** | ✅ | 表格自动转为 Markdown |
| **图片支持** | ✅ | 图片自动保存至本地 |
| **文件附件** | ❌ | 暂不支持 |
| **其他平台** | ❌ | 反爬严格的平台暂不支持（欢迎 PR！） |
...
- **零污染**：CoBridge 自动将同步文件加入 `.gitignore`，确保不会污染你的 Git 仓库。
- **友好格式**：完整 Markdown 输出 — 表格、图片、文本全部保留，IDE Agent 可自然读取。
- **自动配置**：自动生成适配各 Agent 的规则文件，让它们无缝读取上下文。
- **本地优先**：所有数据留在你的机器上，无外部服务器，无遥测。

---

## 📁 项目结构

这是一个 [pnpm workspace](https://pnpm.io/workspaces) Monorepo，包含两个子包：

```
CoBridge/
├── packages/
│   ├── chrome-extension/     # 浏览器插件 (Chrome / Edge)
│   └── vscode-extension/     # VS Code 插件 (Agent 接收端)
├── pnpm-workspace.yaml
└── package.json
```
...
**浏览器里思考，编辑器里落地 — 你的 Agent，从此拥有跨越标签页的长期记忆。**

如果这个项目帮到了你，请在 [GitHub](https://github.com/Winddfall/CoBridge) 上给我们一个 Star ⭐。

## 💡 问题反馈

如果你有新的需求，欢迎在 [GitHub](https://github.com/Winddfall/CoBridge/issues) 上提出 Issue。

## 🤝 参与贡献

如果你有好的建议或发现了 Bug，欢迎提交 Pull Request！
...
<div align="center">
  ...
  <table align="center" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center">
        <img src="../public/assets/wechat-sponsor.png" alt="微信支付" height="160"><br>
        <sub><b>微信支付</b></sub>
      </td>
      <td align="center">
        <img src="../public/assets/alipay-sponsor.jpg" alt="支付宝" height="160"><br>
        <sub><b>支付宝</b></sub>
      </td>
      <td align="center">
