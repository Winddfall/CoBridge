<div align="center">

# <img src="../public/icon.svg" width="15%" alt="" align=center /> Синхронизация памяти ИИ между измерениями ✨

**Думайте в браузере. Реализуйте в редакторе.**

Ваш Agent теперь обладает долгосрочной памятью, выходящей за пределы вкладок.

![Gemini](https://img.shields.io/badge/Gemini-✓-4285F4?style=flat-square&logo=googlegemini&logoColor=white)
![ChatGPT](https://img.shields.io/badge/ChatGPT-✓-000?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![Claude](https://img.shields.io/badge/Claude-✓-D97757?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![deepseek](https://img.shields.io/badge/DeepSeek-✓-4D6BFE?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![豆包](https://img.shields.io/badge/Doubao-✓-A569FF?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)

![github copilot](https://img.shields.io/badge/GitHub%20Copilot-✓-6f42c1?style=flat-square&logo=githubcopilot)
![cursor](https://img.shields.io/badge/Cursor-✓-000000?style=flat-square&logo=cursor)
![antigravity](https://img.shields.io/badge/Antigravity-✓-4285F4?style=flat-square&logo=data:image/svg+xml;base64,)
![trae](https://img.shields.io/badge/Trae-✓-32f08C?style=flat-square&logo=data:image/svg+xml;base64,)
![claude code](https://img.shields.io/badge/Claude%20Code-✓-D97757?style=flat-square&logo=data:image/svg+xml;base64,)
![codex](https://img.shields.io/badge/Codex-✓-5865F2?style=flat-square&logo=data:image/svg+xml;base64,)

[![Version](https://img.shields.io/open-vsx/v/windfall/co-bridge?label=release&style=flat-square)](https://open-vsx.org/extension/windfall/co-bridge)
[![Open VSX Installs](https://img.shields.io/open-vsx/dt/windfall/co-bridge?style=flat-square&label=Open%20VSX%20users)](https://open-vsx.org/extension/windfall/co-bridge)
[![VS Code](https://img.shields.io/badge/users-729%20-blue?style=flat-square&logo=data:image/svg+xml;base64,&logoColor=white)](https://marketplace.visualstudio.com/items?itemName=windfall.co-bridge)
[![License](https://img.shields.io/github/license/Winddfall/CoBridge?style=flat-square)](https://github.com/Winddfall/CoBridge/blob/main/LICENSE)
[![GitHub stars](https://img.shields.io/github/stars/Winddfall/CoBridge?style=flat-square&logo=github)](https://github.com/Winddfall/CoBridge/stargazers)

[English](../README.md) | [简体中文](README_CN.md) | [繁體中文](README_ZH_TW.md) | [日本語](README_JA.md) | [Français](README_FR.md) | [Español](README_ES.md) | [Português](README_PT.md) | [한국어](README_KO.md) | [Русский](README_RU.md) | [العربية](README_AR.md)

</div>

> [!NOTE]
> CoBridge также поддерживает сопутствующее расширение браузера [Voyager](https://github.com/Nagi-ovo/gemini-voyager), разработанное специально для **Gemini**. Если вы используете только Gemini, Voyager — это лёгкая альтернатива. Для более широкой поддержки платформ (ChatGPT, Claude, DeepSeek, Doubao) используйте встроенное расширение браузера CoBridge.
---

## ✨ Возможности

### 📝 Интеллектуальное извлечение контекста
Извлечение контекста бесед ИИ с веб-страниц, включая текст, таблицы и изображения

### 🎯 Автозаполнение системных промптов
Автоматическое заполнение системных промптов для агентов рабочего пространства, быстрый старт проектов

### 🌿 Широкая поддержка экосистемы
Поддержка ChatGPT, Claude, DeepSeek, Doubao и других платформ ИИ

Поддержка GitHub Copilot, Cursor, Claude Code и других агентов IDE

---

## 📥 Установка
### Установка из магазина
#### Шаг 1: Установка расширения браузера
Установите из [Chrome Web Store]() (скоро будет доступно)
#### Шаг 2: Установка расширения VS Code
Откройте Open VSX Marketplace, найдите [**CoBridge**](https://open-vsx.org/extension/windfall/co-bridge) и нажмите «Установить».

### Ручная установка
#### Установка расширения браузера
1. Скачайте или клонируйте репозиторий
2. В корневой директории проекта выполните `pnpm install && pnpm --filter chrome-extension build`
3. Откройте `chrome://extensions/`, включите **Режим разработчика**
4. Нажмите **Загрузить распакованное расширение** и выберите `packages/chrome-extension/dist`

---

## 🏗️ Архитектура

```
┌─────────────────────┐         ┌─────────────────────┐
│  Расширение браузера │  HTTP   │  Расширение VS Code  │
│   (Chrome / Edge)   │ ──────► │  (Приёмник Agent)    │
│                     │ :3030   │                      │
│  Захват чатов ИИ    │         │  Сохранение в рабочий │
│                     │         │ .cobridge/AI_CONTEXT │
└─────────────────────┘         └──────────────────────┘
```

- **Расширение браузера**: внедряется на страницы чатов ИИ, извлекает содержимое диалогов (текст, таблицы, изображения), отправляет на локальный сервер
- **Расширение VS Code**: запускает локальный HTTP-сервер, получает данные и записывает файлы контекста в формате Markdown для чтения агентом IDE

---

## 🚀 Быстрый старт
### Шаг 1: Выбор вашего Agent
![Значок установки](../public/assets/installed.png)
Нажмите на значок панели состояния, чтобы открыть меню управления:

![Меню управления](../packages/vscode-extension/public/assets/management.png)

Выберите используемый Agent:

![Выбор Agent](../packages/vscode-extension/public/assets/select_agent.png)

После выбора на панели состояния отобразится соответствующий значок Agent:

![Agent выбран](../packages/vscode-extension/public/assets/agent_selected.png)

### Шаг 2: Запуск сервиса

В меню нажмите **Запустить сервис**. Локальный сервер будет работать на порту `3030` до ручной остановки.

![Статус работы](../packages/vscode-extension/public/assets/running.png)

### Шаг 3: Синхронизация диалога
![同步对话](../public/assets/browser_extension_en.png)

1. Откройте любую поддерживаемую страницу чата ИИ в браузере
2. Нажмите на значок расширения CoBridge
3. Нажмите **Sync Context to Agent**
4. Содержимое диалога автоматически запишется в `.cobridge/AI_CONTEXT.md` рабочей области

Теперь ваш Agent больше никогда не спросит с недоумением: «О чём вы говорили раньше?»

---

## ⚙️ Конфигурация

### Изменение порта

Если порт `3030` занят:

1. Откройте настройки VS Code (`Ctrl + ,` / `Cmd + ,`)
2. Найдите `AIContextSync.port`
3. Измените номер порта (например, на `3031`)
4. Перезапустите сервис из меню панели состояния

![Настройки порта](../packages/vscode-extension/public/assets/port.png)

> **Примечание:** настройки рабочей области VS Code перекрывают пользовательские настройки. Для изменения порта измените его в настройках рабочей области.

Всплывающее окно расширения браузера также позволяет настроить порт — убедитесь, что настройки совпадают на обоих концах.

---

## 🌐 Поддерживаемые платформы ИИ
![Gemini](https://img.shields.io/badge/Gemini-4285F4?style=for-the-badge&?style=flat-square&logo=googlegemini&logoColor=white)
![ChatGPT](https://img.shields.io/badge/ChatGPT-000000?style=for-the-badge&?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![Claude](https://img.shields.io/badge/Claude-D97757?style=for-the-badge&?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
![deepseek](https://img.shields.io/badge/DeepSeek-4D6BFE?style=for-the-badge&logo=data:image/svg+xml;charset=utf-8;base64,)
![豆包](https://img.shields.io/badge/Doubao-A569FF?style=for-the-badge&?style=flat-square&logo=data:image/svg+xml;charset=utf-8;base64,)
| Платформа | Статус | Описание |
|-----------|--------|----------|
| **Поддержка таблиц** | ✅ | Таблицы автоматически конвертируются в Markdown |
| **Поддержка изображений** | ✅ | Изображения автоматически сохраняются локально |
| **Файловые вложения** | ❌ | Пока не поддерживается |
| **Другие платформы** | ❌ | Платформы со строгой защитой от скрейпинга пока не поддерживаются (PR приветствуются!) |

---

## 🤖 Поддерживаемые Agent

![VS Code](https://img.shields.io/badge/Visual%20Studio%20Code-007ACC?style=for-the-badge&logo=data:image/svg+xml;base64,)
![GitHub Copilot](https://img.shields.io/badge/GitHub%20Copilot-6f42c1?style=for-the-badge&logo=githubcopilot&logoColor=white)
![Cursor](https://img.shields.io/badge/Cursor-000000?style=for-the-badge&logo=cursor&logoColor=white)
![Trae](https://img.shields.io/badge/Trae-32f08C?style=for-the-badge&logo=data:image/svg+xml;base64,)
![Antigravity](https://img.shields.io/badge/Antigravity-4285F4?style=for-the-badge&logo=data:image/svg+xml;base64,)
![Claude Code](https://img.shields.io/badge/Claude%20Code-D97757?style=for-the-badge&logo=data:image/svg+xml;base64,)
![Codex](https://img.shields.io/badge/Codex-5865F2?style=for-the-badge&logo=data:image/svg+xml;base64,)

---

## 🎯 Принципы проектирования

- **Нулевое загрязнение**: CoBridge автоматически добавляет синхронизированные файлы в `.gitignore`, гарантируя отсутствие загрязнения Git-репозитория.
- **Удобный формат**: полный вывод в Markdown — таблицы, изображения, текст сохраняются; агент IDE читает их естественно.
- **Автоматическая настройка**: автоматическая генерация файлов правил для каждого агента, обеспечивающая бесшовное чтение контекста.
- **Локальность**: все данные остаются на вашей машине, без внешних серверов, без телеметрии.

---

## 📁 Структура проекта

Это [pnpm workspace](https://pnpm.io/workspaces) Monorepo, содержащий два подпакета:

```
CoBridge/
├── packages/
│   ├── chrome-extension/     # Расширение браузера (Chrome / Edge)
│   └── vscode-extension/     # Расширение VS Code (Приёмник Agent)
├── pnpm-workspace.yaml
└── package.json
```

---

## 🛠️ Разработка

### Требования

- [Node.js](https://nodejs.org/) >= 20
- [pnpm](https://pnpm.io/) >= 8

### Установка

```bash
git clone https://github.com/Winddfall/CoBridge.git
cd CoBridge
pnpm install
```

### Сборка расширения браузера

```bash
pnpm --filter chrome-extension build
```

### Сборка расширения VS Code

```bash
pnpm --filter vscode-extension compile
```

---

## 🌟 Резюме в одном предложении

**ИИ больше не потеряет память: полноценно думайте на веб-странице, а Agent реализует напрямую.**

Если этот проект помог вам, поставьте звезду на [GitHub](https://github.com/Winddfall/CoBridge).

## 💡 Обратная связь

Если у вас есть предложения, создайте Issue на [GitHub](https://github.com/Winddfall/CoBridge/issues).

## 🤝 Участие в разработке

Если у вас есть идеи или вы нашли баг, добро пожаловать с Pull Request!

## 🥤 Спонсирование проекта
<div align="center">
  <a href="https://github.com/Winddfall/CoBridge">
    <img src="https://raw.githubusercontent.com/winddfall/CoBridge/main/public/assets/sponsors.svg" width="1000px" />
  </a>
</div>
Если этот проект решил вашу проблему совместной работы с ИИ, пригласите меня на стакан колы!

Ваша поддержка напрямую пойдёт на дальнейшее развитие проекта.

<div align="center">

  <p><b>Спонсирование через WeChat / Alipay / Afdian:</b></p>
  <table align="center" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center">
        <img src="../packages/vscode-extension/public/assets/wechat-sponsor.png" alt="WeChat Pay" height="160"><br>
        <sub><b>WeChat Pay</b></sub>
      </td>
      <td align="center">
        <img src="../packages/vscode-extension/public/assets/alipay-sponsor.jpg" alt="Alipay" height="160"><br>
        <sub><b>Alipay</b></sub>
      </td>
      <td align="center">
        <a href="https://afdian.com/a/Wind_fall" target="_blank"><b>Afdian</b></a><br>
        <sub><b>Afdian</b></sub>
      </td>
    </tr>
  </table>
</div>

## 📄 Лицензия

Проект распространяется под лицензией MIT.

## Динамика звёзд

<a href="https://www.star-history.com/?repos=Winddfall%2FCoBridge&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/image?repos=Winddfall/CoBridge&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color: light)" srcset="https://api.star-history.com/image?repos=Winddfall/CoBridge&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/image?repos=Winddfall/CoBridge&type=date&legend=top-left" />
 </picture>
</a>
